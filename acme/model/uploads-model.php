...
function checkExistingImage($imgName){
    ...
    $stmt->bindValue(':imgName', $imgName, PDO::PARAM_STR);
    $stmt->execute();
    $imageMatch = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
    return $imageMatch;
}

//Change default images
function changeImages($invId, $invPath){
    $db = acmeConnect();

    $sql = 'UPDATE inventory SET invImage = :invImage, invThumbnail = :invThumbnail WHERE invId = :invId';

    $stmt = $db->prepare($sql);
    
    $stmt->bindValue(':invThumbnail', makeThumbnailName($invPath), PDO::PARAM_STR);
    $stmt->bindValue(':invImage', $invPath, PDO::PARAM_STR);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
